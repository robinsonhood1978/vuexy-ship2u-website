        ...
        <b-form @submit.prevent>
          ...
          >
            ...
            <b-card-body class="parcel-padding pb-0">
              <b-row v-if="!parcelData.receive_time">
                <b-col cols="12">
                  <b-form-group
                    :label="`${$t('title')}`"
              ...
              <div
                v-if="parcelData.receive_time"
                class="d-flex justify-content-between flex-md-row flex-column parcel-spacing mt-0"
              >
...
import {
  BRow, BCol, BCard, BCardTitle, BCardSubTitle, BCardBody, BButton, BForm, BFormGroup, BFormInput, BInputGroup, BInputGroupText, BInputGroupPrepend, BFormTextarea, BAlert, BLink, VBToggle,
} from 'bootstrap-vue'
...
export default {
  ...
  watch: {
    ...
    'parcelData.orderdetail_set': function () {
      ...
      this.freightType = this.ftypes.filter(item => item.code === this.parcelData.freight_type)[0]
      this.initTrHeight()
    },
  },
  mounted() {
  ...
  setup() {
    ...
    store.dispatch('ship2u/fetchParcel', { id: router.currentRoute.params.id })
      .then(response => {
        ...
        if (parcelData.value.orderdetail_set.length < 1) {
          parcelData.value.orderdetail_set.push(JSON.parse(JSON.stringify(itemFormBlankItem)))
        }
      })
      .catch(error => {
    ...
    function onPostParcel() {
      ...
      store.dispatch('ship2u/saveParcel', { obj: formData.value, parcel: parcelData.value })
        .then(response => {
          ...
          this.$bvModal.msgBoxOk(this.$i18n.t('Data was submitted successfully'), {
