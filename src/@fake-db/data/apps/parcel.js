...
// GET: Return Users
// ------------------------------------------------
mock.onGet('/apps/ship2u/parcels').reply(config => {
  // eslint-disable-next-line object-curly-newline
  const { q = '', perPage = 10, page = 1, sortBy = 'id', sortDesc = false, status = null } = config.params
  /* eslint-enable */

  const queryLowered = q.toLowerCase()
  const filteredData = data.parcels.filter(
    parcel =>
      /* eslint-disable operator-linebreak, implicit-arrow-linebreak */
      (parcel.client.companyEmail.toLowerCase().includes(queryLowered) ||
        parcel.client.name.toLowerCase().includes(queryLowered)) &&
      parcel.parcelStatus === (status || parcel.parcelStatus),
  )
  /* eslint-enable  */
...
// ------------------------------------------------
// GET: Return Single parcel
// ------------------------------------------------
mock.onGet(/\/apps\/ship2u\/parcels\/\d+/).reply(config => {
  // Get event id from URL
  let parcelId = config.url.substring(config.url.lastIndexOf('/') + 1)

  // Convert Id to number
  parcelId = Number(parcelId)

  const parcelIndex = data.parcels.findIndex(e => e.id === parcelId)
  ...
  if (parcel) return [200, responseData]
  return [404]
})
...
// GET: Return Clients
// ------------------------------------------------
mock.onGet('/apps/ship2u/clients').reply(() => {
  const clients = data.parcels.map(parcel => parcel.client)
